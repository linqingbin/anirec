...
import pymongo
import pdb
from flask import Flask, render_template, flash, request, redirect, url_for, jsonify, abort, g
...
def setUp():
    ...
    g.db = pymongo.MongoClient(app.config["DBHOST"], 27017)
...
def index():
    ...
    user_id = int(request.args.get('page_num')) if 'user_id' in request.args else 0
    if user_id == 0:
    ...
    return render_template('index.html', animelist=list(animelist), r_content=r_content)
...
def isviewed_update():
    ...
    print(r_data)
    anime = g.db['anime']
    anime.animeinfo.update_one({'id': r_data['anime_id']}, {'$set': {'isviewed': r_data['isviewed']}})
    return ('', 204)
...
def animelist():
    page_num = int(request.args.get('page_num')) if 'page_num' in request.args else 1
    user_id = int(request.args.get('user_id')) if 'user_id' in request.args else 0
    filter_paras = ['start_year', 'end_year', 'season','status','keyword']
    start_year, end_year, season, status, keyword = [request.args.get(x) for x in filter_paras]
    ...
    animelist = anime.animeinfo.find({
        ...
        "$or":[
            {"title_japanese": {'$regex':'.*{}.*'.format(keyword), "$options": 'i'}},
            {"title_romaji": {'$regex':'.*{}.*'.format(keyword),"$options": 'i'}}
            ]
    },
        {
            "_id": 0,
